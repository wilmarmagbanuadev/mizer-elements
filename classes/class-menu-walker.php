...
namespace BlankElementsPro\Classes;
...
class Menu_Walker extends \Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 ) {
		...
		$submenu = $args->has_children ? ' be-has-submenu' : '';

		if ( 0 === $depth ) {
			array_push( $classes, 'parent' );
		}
		$class_names = join( ' ', apply_filters( 'nav_menu_css_class', array_filter( $classes ), $item, $args, $depth ) );
		$class_names = ' class="' . esc_attr( $class_names ) . $submenu . ' be-creative-menu"';
		...
		$item_output  = $args->has_children ? '<div class="be-has-submenu-container">' : '';
		$item_output .= $args->before;
		$item_output .= '<a' . $attributes;
		if ( 0 === $depth ) {
			$item_output .= ' class = "be-menu-item"';
		} else {
			$item_output .= in_array( 'current-menu-item', $item->classes ) ? ' class = "be-sub-menu-item be-sub-menu-item-active"' : ' class = "be-sub-menu-item"';
		}

		$item_output .= '>';
		$item_output .= $args->link_before . apply_filters( 'the_title', $item->title, $item->ID ) . $args->link_after;
		if ( $args->has_children ) {
			$item_output .= "<span class='be-menu-toggle sub-arrow be-menu-child-";
			$item_output .= $depth;
